...
> **Unaffected by the 2026-07-06 spec revision.** That revision (one Lead per activity, QC as an `is_qc` flag, Head-managed staffing, PM staffing-read-only, activity-grouped UI) reshapes **Phase 3** (`docs/superpowers/plans/2026-07-06-phase3-activity-assignments.md`). Phase 2's surface — `head_employee_id`, the project-level `authz` helpers (`can_view_project`/`can_manage_project`/`can_assign_head`/`can_review_report`), the Head-assign API, and the Head panel — does not touch the Lead/QC model, so this plan proceeds as written.
...
- **Product decisions (CONFIRMED 2026-07-06):** (a) Head picker allows **any employee** (not only project members); the assignment service **auto-maintains `project_members`** (adds the chosen Head as a member). (b) `project_managers` stays as a fallback exactly per the architecture: notification routing is **Head → `reporting_pm_id` → `project_managers`** until Phase 6 removes the legacy mechanism.
