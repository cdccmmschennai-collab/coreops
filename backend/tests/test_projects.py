...
def test_single_lead_enforced(client, auth_header, make_project, make_employee):
    ...
    client.post(f"/api/v1/projects/{p.id}/members", headers=h, json={"employee_id": str(e1.id), "role": "team_lead"})
    client.post(f"/api/v1/projects/{p.id}/members", headers=h, json={"employee_id": str(e2.id), "role": "team_lead"})
    members = client.get(f"/api/v1/projects/{p.id}/members", headers=h).json()
    leads = [m for m in members if m["role"] == "team_lead"]
    assert len(leads) == 1
...
def test_project_manager_sees_all_projects_and_can_create(
    client, make_user, make_employee, make_project, make_project_member, login
):
    user = make_user("mgr@example.com", role=UserRole.project_manager)
    emp = make_employee(employee_code="MGR", user_id=user.id)
    make_project(code="MGR-ASSIGNED", status=ProjectStatus.active)
    make_project(code="MGR-OTHER", status=ProjectStatus.active)
    h = login("mgr@example.com")
    res = client.get("/api/v1/projects", headers=h).json()
    # project_manager sees ALL projects (no assignment scoping)
    assert {"MGR-ASSIGNED", "MGR-OTHER"}.issubset({p["code"] for p in res["items"]})
    # project_manager can create projects
    assert client.post("/api/v1/projects", headers=h, json=_payload(code="MGR-NEW")).status_code == 201
