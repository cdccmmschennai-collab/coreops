...
# Applies to EVERY kind of work item, lump-sum included: the weekly boundary is
# intentional product behaviour, and work-day counting measures the allowed
# duration WITHIN a reporting week rather than extending an item past it.
# --------------------------------------------------------------------------
...
def test_same_activity_new_cycle_creates_separate_item(flag_on, client, author, pm_header, db):
    ...
    reusing the previous-cycle one, which stays incomplete. The sub-activity here
    is lump-sum (a bare _task_sub has no relevant_count_field), so this also
    pins that a lump-sum activity gets a NEW allowance in a new reporting week
