...
# ── the conditional requirement: a count must name its field ─────────────────


def test_no_count_and_no_field_saves(client, setup_author, pm_header):
    """The Count row is optional in BOTH halves. An activity with nothing to
    count is ordinary and must stay saveable — this is the case that must never
    regress."""
    a = setup_author()
    _, sub = _lumpsum_sub(client, pm_header)

    created = _create(
        client, a["header"], [_task_row(a["project"].id, sub["id"])],
    )
...
def test_a_count_with_no_field_is_rejected(client, setup_author, pm_header):
    """25 typed with the picker left on "Select field". The server will not
    guess a column, so it refuses rather than filing the number somewhere."""
    a = setup_author()
    _, sub = _lumpsum_sub(client, pm_header)
...
def test_a_count_with_its_field_saves(client, setup_author, pm_header):
    """The same 25, once it says which field it is. Sent as the explicit
    count_field/count_value pair, which lands in that field's own column."""
    a = setup_author()
    _, sub = _lumpsum_sub(client, pm_header)
...
def test_a_zero_count_is_no_count(client, setup_author, pm_header):
    """0 is what an untouched unit already reads, so it is not a count waiting
    for a field — the row saves with the picker still empty."""
    a = setup_author()
    _, sub = _lumpsum_sub(client, pm_header)

    created = _create(
        client, a["header"],
        [_task_row(a["project"].id, sub["id"], count_value=0)],
    )

    assert created["tasks"][0]["relevant_count_field_snapshot"] is None


def test_a_field_with_no_count_is_not_stored(client, setup_author, pm_header):
    """Picking a field is a statement ABOUT a count. With no count there is
    nothing to state, so the pick is dropped rather than persisted as a field
    with an empty value — which would export an artificial 0."""
    a = setup_author()
    _, sub = _lumpsum_sub(client, pm_header)

    created = _create(
        client, a["header"],
        [_task_row(a["project"].id, sub["id"], count_field="tags")],
    )
...
def test_a_field_with_an_explicit_zero_is_not_stored(client, setup_author, pm_header):
    a = setup_author()
    _, sub = _lumpsum_sub(client, pm_header)
...
def test_an_unattributed_count_does_not_block_other_counts(
    client, setup_author, pm_header
):
    """"Other counts" are attributed by construction — each is typed into its
    own named input — so a lumpsum row carrying only those still saves with no
    field picked."""
    a = setup_author()
    _, sub = _lumpsum_sub(client, pm_header)
    ...
    task = created["tasks"][0]
    assert task["relevant_count_field_snapshot"] is None
    assert (task["docs_count"], task["bom_count"]) == (10, 5)


def test_a_countless_row_still_submits(client, setup_author, pm_header):
    """The whole lifecycle, not just the save: a lumpsum with no count at all
    must still reach submitted."""
    a = setup_author()
    _, sub = _lumpsum_sub(client, pm_header)
    ...
    assert submitted["tasks"][0]["relevant_count_field_snapshot"] is None


def test_a_countless_row_exports_no_artificial_count(
    client, setup_author, pm_header
):
    a = setup_author()
    _, sub = _lumpsum_sub(client, pm_header)
    ...
    # Every unit column reads 0 — the sheet's own "nothing counted" — and no
    # column was singled out to carry an invented figure.
    for header in ("NO. OF TAGS", "NO. OF DOCS", "NO. OF BOM HEADER",
                   "NO. OF SPARES", "NO. OF PAGES", "NO. OF RECORDS"):
        assert cell[header] == 0
...
def test_omitting_the_field_leaves_a_plain_lumpsum_row(client, setup_author, pm_header):
    """A lumpsum recorded the way it always was â€” no count, no unit â€” is
    unchanged."""
    a = setup_author()
    _, sub = _lumpsum_sub(client, pm_header)
    ...
    assert task["relevant_count_field_snapshot"] is None
    assert task["tags_count"] == 0
    assert task["benchmark_type_snapshot"] == "TASK_STATUS_ONLY"
