...
export function DashboardView() {
  ...
  // for manager: pending-review count from team
  const pendingReview = useWorkReportList({
    employee_id: "",
    project_id: "",
    status: "submitted",
    from: "",
  ...
  const teamPending = pendingReview.data?.total ?? 0;
  ...
  return (
    <>
      ...
      <KpiGrid>
        ...
        />
        {isManagerial(role) ? (
          <Kpi label="Pending review" value={String(teamPending)} />
        ) : (
          <Kpi label="In review" value={String(inReview)} />
        )}
        <Kpi
          label={isManagerial(role) ? "My submitted" : "Approved"}
          value={String(
            isManagerial(role)
              ? weekItems.filter((r) => r.status === "submitted").length
              : weekItems.filter((r) => r.status === "approved").length,
          )}
        />
      </KpiGrid>
        ...
        <Card>
          <CardHeader className="border-b border-border px-5 py-3.5">
            <CardTitle className="text-base">
              {isManagerial(role) ? "Pending team reviews" : "Quick actions"}
            </CardTitle>
          </CardHeader>
                ...
                {pendingReview.isLoading ? (
                  ...
                  <>
                    <p className="text-sm text-muted-foreground">
                      <span className="tabular font-semibold text-foreground">{teamPending}</span>{" "}
